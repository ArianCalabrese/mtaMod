...
addEventHandler("onResourceStart", _root, function(resource)
    ...
    if (node) then
        ...
        table.sort(aReports, function(a, b)
...
function aAddUnmuteTimer(player, length)
    aRemoveUnmuteTimer(player)
    local serial = getPlayerSerial(player)
    aUnmuteTimerList[serial] = setTimer(function()
        aUnmuteTimerList[serial] = nil
...
function isPlayerFrozen(player)
    if (aPlayers[player]["freeze"] == nil) then
        aPlayers[player]["freeze"] = false
    end
...
addEventHandler("onPlayerJoin", _root, function()
    ...
    for id, player in ipairs(getElementsByType("player")) do
        if (hasObjectPermissionTo(player, "general.adminpanel")) then
            triggerClientEvent(player, "aClientPlayerJoin", source, getPlayerIP(source), getPlayerUserName(source),
                getPlayerAccountName(source), getPlayerSerial(source),
                hasObjectPermissionTo(source, "general.adminpanel"), aPlayers[source]["country"])
        end
    end
...
addEventHandler("aPlayerVersion", _root, function(version)
    if checkClient(false, source, 'aPlayerVersion') then
        return
    end
    local bIsPre = false
...
addCommandHandler("register", function(player, command, arg1, arg2)
    ...
    if (password ~= nil) then
        ...
        elseif (addAccount(username, password)) then
            outputChatBox(
                "You have successfully registered! Username: '" .. username .. "', Password: '" .. password ..
                    "'(Remember it)", player, 255, 100, 70)
            outputServerLog("ACCOUNTS: " .. getPlayerName(player) .. " registered account '" .. username .. "' (IP: " ..
                                getPlayerIP(player) .. "  Serial: " .. getPlayerSerial(player) .. ")")
...
addCommandHandler("unregister", function(player, command, arg1, arg2)
    ...
    outputServerLog(
        "ADMIN: " .. getAdminNameForLog(player) .. " unregistering account '" .. username .. "' " .. result .. " (IP: " ..
            getPlayerIP(player) .. "  Serial: " .. getPlayerSerial(player) .. ")")
end)
...
addEventHandler("aTeam", _root, function(action, name, r, g, b)
    ...
    if (hasObjectPermissionTo(client or source, "command." .. action)) then
        ...
        if (action ~= nil) then
            aAction("server", action, source, false, mdata, mdata2)
        end
        return true
    end
        ...
        if (cmd == "change") then
            ...
            if changed then
                if aSetResourceSetting(resName, name, value) then
                    ...
                    mdata = resName .. '.' .. name
                    mdata2 = type(value) == "table" and string.gsub(toJSON(value), "^(%[ %[ )(.*)( %] %])$", "%2") or
                                 tostring(value)
                end
            end
        ...
        if ((name) and (string.len(name) >= 1)) then
            if (arg[1] == "group") then
                ...
                if (not aclCreateGroup(name)) then
                    ...
                    outputServerLog(
                        "ACL: " .. getPlayerName(source) .. "[" .. getAccountName(getPlayerAccount(source)) .. "] [" ..
                            getPlayerSerial(source) .. "] [" .. getPlayerIP(source) .. "] created " .. mdata)
                end
    ...
    elseif (action == "acldestroy") then
        ...
        if (arg[1] == "group") then
            if (aclGetGroup(name)) then
                ...
                outputServerLog("ACL: " .. getPlayerName(source) .. "[" .. getAccountName(getPlayerAccount(source)) ..
                                    "] [" .. getPlayerSerial(source) .. "] [" .. getPlayerIP(source) .. "] destroyed " ..
                                    mdata)
            else
                action = nil
            end
        ...
        if (arg[3]) then
            ...
            if (arg[1] == "object") then
                ...
                if (not aclGroupAddObject(group, object)) then
                    action = nil
                    outputChatBox("Error adding object '" .. tostring(object) .. "' to group '" .. tostring(arg[2]) ..
                                      "'", source, 255, 0, 0)
                else
                    mdata2 = "Object '" .. arg[3] .. "'"
                    outputServerLog(
                        "ACL: " .. getPlayerName(source) .. "[" .. getAccountName(getPlayerAccount(source)) .. "] [" ..
                            getPlayerSerial(source) .. "] [" .. getPlayerIP(source) .. "] added " .. object ..
                            " to ACL Group " .. arg[2])
            ...
            elseif (arg[1] == "acl") then
                ...
                if (not aclGroupAddACL(group, acl)) then
                    action = nil
                    outputChatBox("Error adding ACL '" .. tostring(arg[3]) .. "' to group '" .. tostring(arg[2]) .. "'",
                        source, 255, 0, 0)
                else
                    mdata2 = "ACL '" .. arg[3] .. "'"
                    triggerEvent("aAdmin", source, "sync", "aclobjects", arg[2])
                    outputServerLog(
                        "ACL: " .. getPlayerName(source) .. "[" .. getAccountName(getPlayerAccount(source)) .. "] [" ..
                            getPlayerSerial(source) .. "] [" .. getPlayerIP(source) .. "] added " .. mdata2 ..
                            " to Group " .. arg[2])
                end
            elseif (arg[1] == "right") then
                ...
                if (not aclSetRight(acl, right, enabled)) then
                    action = nil
                    outputChatBox("Error adding right '" .. tostring(arg[3]) .. "' to group '" .. tostring(arg[2]) ..
                                      "'", source, 255, 0, 0)
                else
                    mdata2 = "Right '" .. arg[3] .. "'"
        ...
        if (arg[3]) then
            ...
            if (arg[1] == "object") then
                ...
                if (not aclGroupRemoveObject(group, object)) then
                    action = nil
                    outputChatBox("Error - object '" .. tostring(object) .. "' does not exist in group '" ..
                                      tostring(arg[2]) .. "'", source, 255, 0, 0)
                else
                    mdata2 = "Object '" .. arg[3] .. "'"
                    triggerEvent("aAdmin", source, "sync", "aclobjects", arg[2])
                    outputServerLog(
                        "ACL: " .. getPlayerName(source) .. "[" .. getAccountName(getPlayerAccount(source)) .. "] [" ..
                            getPlayerSerial(source) .. "] [" .. getPlayerIP(source) .. "] removed " .. mdata2)
                end
            elseif (arg[1] == "acl") then
                local group = aclGetGroup(arg[2])
                local acl = aclGet(arg[3])
                if (not aclGroupRemoveACL(group, acl)) then
                    action = nil
                    outputChatBox("Error - ACL '" .. tostring(arg[3]) .. "' does not exist in group '" ..
                                      tostring(arg[2]) .. "'", source, 255, 0, 0)
                else
                    mdata2 = "ACL '" .. arg[3] .. "'"
                    triggerEvent("aAdmin", source, "sync", "aclobjects", arg[2])
            ...
            elseif (arg[1] == "right") then
                ...
                if (not aclRemoveRight(acl, right)) then
                    action = nil
                    outputChatBox("Error - right '" .. tostring(arg[3]) .. "' does not exist in ACL '" ..
                                      tostring(arg[2]) .. "'", source, 255, 0, 0)
                else
                    mdata = "ACL '" .. arg[2] .. "'"
                    mdata2 = "Right '" .. arg[3] .. "'"
...
addEventHandler("aPlayer", _root, function(player, action, data, additional, additional2)
    if checkClient("command." .. action, source, 'aPlayer', action) then
        return
    end
    ...
    if (hasObjectPermissionTo(client or source, "command." .. action)) then
        ...
        elseif (action == "setnick") then
            ...
            if setPlayerName(player, data) then
                ...
                outputChatBox("'" .. getPlayerName(source) .. "' changed your nick to '" .. data .. "'!", player, 255,
                    100, 70)
            else
        ...
        elseif (action == "jetpack") then
            if (doesPedHaveJetPack(player)) then
                ...
                if (getPedOccupiedVehicle(player)) then
                    outputChatBox("Unable to give a jetpack - " .. getPlayerName(player) .. " is in a vehicle", source,
                        255, 0, 0)
                else
        ...
        elseif (action == "setgroup") then
            ...
            if (not isGuestAccount(account)) then
                ...
                if (group) then
                    ...
                    outputChatBox("Error - Admin group not initialized. Please reinstall admin resource.", source, 255,
                        0, 0)
                end
            else
            ...
            function warpPlayer(p, to)
                if (isElement(to) and isPedInVehicle(to)) then
                    ...
                    if (i >= seats) then
                        warp(p, to)
                        outputConsole(
                            "Player's vehicle is full (" .. getVehicleName(vehicle) .. " - Seats: " .. seats .. ")", p)
                    end
                else
                    warp(p, to)
                end
        ...
        if (action ~= nil) then
            aAction("player", action, admin, player, mdata, more)
        end
        return true
    end
    outputChatBox("Access denied for '" .. tostring(action) .. "'", source, 255, 168, 0)
...
addEventHandler("aVehicle", _root, function(player, action, data)
    if checkClient("command." .. action, source, 'aVehicle', action) then
        return
    end
    if (hasObjectPermissionTo(client or source, "command." .. action)) then
        if (not player) then
...
addEventHandler("aResource", _root, function(name, action)
    if checkClient("command." .. action, source, 'aResource', action) then
        return
    end
    ...
    if (hasObjectPermissionTo(client or source, "command." .. action)) then
        ...
        elseif (action == "restart") then
            if (getResourceState(getResourceFromName(name)) == "running") then
                if (restartResource(getResourceFromName(name))) then
                    text = "restarted"
                end
            end
...
addEventHandler("aServer", _root, function(action, data, data2)
    if checkClient("command." .. action, source, 'aServer', action) then
        return
    end
    if (hasObjectPermissionTo(client or source, "command." .. action)) then
...
addEventHandler("aMessage", _root, function(action, data)
    ...
    if (action == "new") then
        ...
        aReports[id].time = string.format('%04d-%02d-%02d %02d:%02d', time.year + 1900, time.month + 1, time.monthday,
        ...
        for _, p in ipairs(getElementsByType("player")) do
            if (hasObjectPermissionTo(p, "general.adminpanel")) then
                outputChatBox("New Admin message from " .. aReports[id].author .. " (" .. aReports[id].subject .. ")",
                    p, 255, 0, 0)
            end
        end
    ...
    if (hasObjectPermissionTo(client or source, "general.adminpanel")) then
        ...
        elseif (action == "delete") then
            if (aReports[data]) then
                outputServerLog("ADMIN: " .. getPlayerName(client) .. " has deleted a report with subject '" ..
                                    aReports[data].subject .. "'.")
...
addEventHandler("aModdetails", resourceRoot, function(action, player)
    if checkClient(false, client, 'aModdetails', action) then
        return
    end
...
addEventHandler("aBans", _root, function(action, data, arg1, arg2, arg3)
    if checkClient("command." .. action, source, 'aBans', action) then
        return
    end
    if (hasObjectPermissionTo(client or source, "command." .. action)) then
        local mdata = ""
...
addEventHandler("aExecute", _root, function(action, echo)
    if checkClient("command.execute", source, 'aExecute', action) then
        return
    end
...
addEventHandler("aAdminChat", _root, function(chat)
    if checkClient(true, source, 'aAdminChat') then
        return
    end
    for id, player in ipairs(getElementsByType("player")) do
...
addCommandHandler(get("adminChatCommandName"), function(thePlayer, cmd, ...)
...
function checkClient(checkAccess, player, ...)
    if client and client ~= player and g_Prefs.securitylevel >= 2 then
        ...
        outputDebugString("Admin security - Client/player mismatch from " .. tostring(ipAddress) .. " (" ..
                              tostring(desc) .. ")", 1)
        cancelEvent()
    ...
    if checkAccess and g_Prefs.securitylevel >= 1 then
        if type(checkAccess) == 'string' then
            ...
            if hasObjectPermissionTo(player, "general.adminpanel") then
                outputDebugString("Admin security - Client does not have required rights (" .. checkAccess .. "). " ..
                                      tostring(ipAddress) .. " (" .. tostring(desc) .. ")")
        ...
        if not hasObjectPermissionTo(player, "general.adminpanel") then
            ...
            outputDebugString("Admin security - Client without admin panel rights trigged an admin panel event. " ..
                                  tostring(ipAddress) .. " (" .. tostring(desc) .. ")", 2)
...
function checkNickOnChange(old, new)
    if aNickChangeTime[source] and aNickChangeTime[source] + tonumber(get("*nickChangeDelay")) > getTickCount() then
        cancelEvent()
        outputChatBox("You can only change your name once every " .. (tonumber(get("*nickChangeDelay")) / 1000) ..
                          " seconds", source, 255, 0, 0)
        return false
    else
...
addEventHandler("onUnban", root, function(theBan, responsibleElement)
    if isElement(responsibleElement) and getElementType(responsibleElement) == "player" then
        outputServerLog("BAN: " .. getPlayerName(responsibleElement) .. "[" ..
                            getAccountName(getPlayerAccount(responsibleElement)) .. "] [" ..
                            getPlayerSerial(responsibleElement) .. "] [" .. getPlayerIP(responsibleElement) ..
                            "] unbanned player " .. (getBanNick(theBan) or "unknown") .. " [" ..
                            (getBanIP(theBan) or getBanSerial(theBan)) .. "] ")
    end
end)

addEventHandler("onBan", root, function(theBan)
    if isElement(source) and getElementType(source) == "player" then
        outputServerLog("BAN: " .. getPlayerName(source) .. "[" .. getAccountName(getPlayerAccount(source)) .. "] [" ..
                            getPlayerSerial(source) .. "] [" .. getPlayerIP(source) .. "] banned player " ..
                            (getBanNick(theBan) or "unknown") .. " [" .. (getBanIP(theBan) or getBanSerial(theBan)) ..
                            "] ")
    end
end)
